...
| 9 | Country filter depends on `competition.area.name` | `implemented` | ESPN emits no `area` at all, so a verified registry supplies it: `data/competition-countries.json` (15 domestic leagues; multi-country competitions deliberately unmapped) + `services/competitions.py`. The control hides below two resolvable countries. `tests/test_competition_registry.py`, `tests/browser/country-filter.spec.js` |
