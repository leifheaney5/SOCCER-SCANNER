...
| 7 | Standings season hardcoded | `implemented` | `soccer_scanner/data/standings-seasons.json` + `services/standings.py`; template renders from configuration. `staleAfterDays: 330` trips 2027-07-03, before the next pre-season, and `tests/test_standings_seasons.py::test_configuration_is_not_stale_today` fails when it does |
...
| 9 | Country filter depends on `competition.area.name` | `implemented_with_follow_up` | Machinery works and is tested (`tests/test_competition_registry.py`, `tests/test_competition_enrichment.py`, `tests/browser/country-filter.spec.js`); the empty non-functional control is gone — it now hides below two resolvable countries. **But it resolves almost nothing against real data.** Measured on production 2026-08-06: **0 of 64 fixtures** resolved a country. ESPN emits `canonicalId: null` for nearly every competition and country-prefixes its names (`'English Carabao Cup'`, `'Venezuelan Primera División'`, `'Bolivian Liga Profesional'`, `'Argentine Liga Profesional de Fútbol'`), which the alias list does not match. Follow-up in Phase 4: resolve the country from ESPN's prefix convention, verified against captured payloads rather than assumed names |
...
| PWA excludes ET/PEN from offline cache | `implemented` | `offline-cache.js` filters by `isOfflineEligible`; `tests/offline-cache.test.mjs` |
| Recompute counts/featured after sanitization | `not_applicable` | Investigated: no client code reads `featured_matches` or `matchStatistics` — `renderFeatured` derives from the filtered list via `selectFeatured`, and the only `byTimeSlot` reference is a server-side test. The inconsistency this row describes cannot occur. The stale fields are dropped from the snapshot instead of recomputed; `tests/offline-cache.test.mjs` |
| Team Intelligence partial states | `partial` | `services/team_analytics.py`, `team_identity.py` (102 lines, ~6 mappings) |
| Sorting not overwritten by grouping | `implemented` | `groupMatches(matches, sort)` preserves insertion order; fixed in phase 1 |
| `popstate` restores fixture context | `implemented` | Restores the fixture the URL names and reconciles the panel only when the selection changes. `tests/browser/history-restoration.spec.js` — 7 tests incl. select-A-then-B-then-Back and a failed refetch, both confirmed failing pre-fix |
